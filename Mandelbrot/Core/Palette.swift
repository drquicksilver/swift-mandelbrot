...
enum Palette: String, CaseIterable, Identifiable, Sendable, Codable {
  ...
  func rgb(_ phase: Float) -> SIMD3<Float> {
    ...
    if self == .orbit {
      ...
      let a = 0.10 * cos(angle)
      let b = 0.10 * sin(angle)
      let l: Float = 0.72
      ...
      func encode(_ x: Float) -> Float {
        min(1, max(0, x <= 0.0031308 ? x * 12.92 : 1.055 * pow(x, 1 / 2.4) - 0.055))
      }
      return SIMD3(encode(linear.x), encode(linear.y), encode(linear.z))
    }
    let position = t * Float(stops.count - 1)
    let index = Int(position)
    let f = position - Float(index)
