...
struct Viewport: Equatable, Sendable {
    ...
    @discardableResult mutating func zoom(by factor: Double, at anchor: CGPoint,
                                          in size: CGSize, pixelWidth: Double) -> Bool {
    ...
    mutating func fit(_ rect: CGRect, in size: CGSize, pixelWidth: Double) {
        ...
        zoom(by: factor, at: CGPoint(x: size.width/2, y: size.height/2), in: size, pixelWidth: pixelWidth)
        center = fixedPoint
    }
