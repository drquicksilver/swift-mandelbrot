...
@MainActor enum TileCompositor {
    static func uv(cell: TileBounds,source: TileBounds) -> SIMD4<Float> {
        let u=(cell.left-source.left)/source.span,v=(source.top-cell.top)/source.span,extent=cell.span/source.span
        let resolution=Double(TileGrid.textureSize),interior=Double(TileGrid.samples)
        return SIMD4(Float((1+u*interior)/resolution),Float((1+v*interior)/resolution),
    ...
    static func encode(store: TileStore,viewport: Viewport,size: CGSize,gpu: GPUContext,encoder: MTLRenderCommandEncoder,
        ...
        for key in store.visible {
            ...
            let coarse=(base.key.level>store.minimumLevel ? store.bestAvailable(for:base.key.parent) : nil) ?? base
            let fine=store.records[key] ?? base
            let cell=store.grid.bounds(key),center=viewport.screen(for:cell.center,in:size),width=cell.span/viewport.span*size.width
            ...
            params.coarseUV=uv(cell:cell,source:coarse.bounds);params.baseUV=uv(cell:cell,source:base.bounds);params.fineUV=uv(cell:cell,source:fine.bounds)
            params.baseMix=coarse === base ? 1 : TilePresentation.fade(readyAt:base.readyAt,now:now)
            params.fineMix=fine === base ? 0 : TilePresentation.fineWeight(lod:store.lod,readyAt:fine.readyAt,now:now)
            params.border=overlay ? 1 : 0;params.level=Int32(base.key.level)
            ...
            encoder.setFragmentTexture(coarse.colour,index:0);encoder.setFragmentTexture(base.colour,index:1);encoder.setFragmentTexture(fine.colour,index:2)
            encoder.drawPrimitives(type:.triangle,vertexStart:0,vertexCount:6)
        }
    }
    static func snapshot(store: TileStore,viewport: Viewport,width: Int,height: Int, sentinel: Bool = false,now: Double = ProcessInfo.processInfo.systemUptime,overlay: Bool = false) async throws -> MTLTexture {
        ...
        encode(store:store,viewport:viewport,size:CGSize(width:width,height:height),gpu:gpu,encoder:encoder,now:now,overlay:overlay)
