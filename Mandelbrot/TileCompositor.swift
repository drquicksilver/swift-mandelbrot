...
@MainActor enum TileCompositor {
    static func encode(store: TileStore,viewport: Viewport,size: CGSize,gpu: GPUContext,encoder: MTLRenderCommandEncoder) {
        ...
        for key in store.visible {
            ...
            let low=Float(TileGrid.gutter)/Float(TileGrid.textureSize),high=1-low
            params.uv=SIMD4(low,low,high,high)
    ...
    static func snapshot(store: TileStore,viewport: Viewport,width: Int,height: Int) async throws -> MTLTexture {
        ...
        descriptor.colorAttachments[0].clearColor=MTLClearColor(red:0.01,green:0.01,blue:0.02,alpha:1)
