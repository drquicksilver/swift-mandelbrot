        ...
        NavigationStack {
            Form {
                Section("Rendering") {
                    ...
                    Text("Automatic: \(model.viewport.recommendedRenderer(pixelWidth:model.pixelWidth).title)").font(.caption)
                }
            ...
            .sheet(isPresented:$benchmarks) { BenchmarkView(viewport:model.viewport,iterations:model.iterations) }
    }
}
