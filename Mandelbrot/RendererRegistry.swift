...
enum RendererRegistry {
  private typealias CPUFunction = (Int, Int, CGPoint, Double, Int, MandelbrotConfiguration) ->
  ...
  static func renderer(for id: RendererID) -> any Renderer {
    ...
    return RegisteredRenderer(id: id) { request in
      ...
      let render =
        id == .metal ? MandelbrotMetalRenderer.iterations : MandelbrotMetalRenderer.iterationsDouble
      return render(
        request.width, request.height, request.viewport.center, request.viewport.scale,
