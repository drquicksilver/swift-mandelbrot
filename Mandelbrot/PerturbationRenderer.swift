...
extension GPUContext {
  ...
  func perturb(into samples: MTLTexture, region: PerturbationRegion, iterations: Int) async throws
    -> PerturbationMetrics
  {
    ...
    guard
      ...
      let flags = device.makeBuffer(length: 16, options: .storageModeShared)
    else { throw GPUFailure("Perturbation allocation failed") }
    var referencePoint = region.point(x: region.width / 2, y: region.height / 2)
    for pass in 0..<16 {
      try Task.checkCancellation()
      let point = referencePoint
      let task = Task.detached(priority: .userInitiated) {
        try ReferenceOrbit.compute(point: point, iterations: iterations, bits: region.bits)
      }
      let reference = try await withTaskCancellationHandler {
        try await task.value
      } onCancel: {
        task.cancel()
      }
      metrics.referenceSeconds += reference.seconds
      ...
      let words = flags.contents().bindMemory(to: UInt32.self, capacity: 4)
      words[0] = UInt32.max
      words[1] = 0
      words[2] = 0
      words[3] = 0
      ...
      var p = PerturbationParameters(
        ...
        start: 0, count: 32, pass: UInt32(pass), padding: 0)
      var batch = 32
      while p.start < iterations {
        ...
        p.start += p.count
        batch = max(1, min(128, Int(Double(p.count) * min(2, 0.001 / max(seconds, 0.00001)))))
      }
