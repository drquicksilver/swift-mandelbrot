...
@Test func inertiaIsRefreshRateIndependent() {
    func travel(hz: Int) -> (Double,Double) {
        var motion=Motion(velocity:SIMD2(1000,0),zoomVelocity:2), x=0.0,scale=1.0
        for _ in 0..<(hz/2) { let delta=motion.step(seconds:1/Double(hz));x+=delta.pan.width;scale*=delta.zoom }
        return (x,scale)
    }
    let a=travel(hz:60),b=travel(hz:120)
