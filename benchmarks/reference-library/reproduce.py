...
def main():
    ...
    parser.add_argument('--output', type=pathlib.Path, default=ROOT/'docs/evidence/perturbation/review/libraries.json')
    ...
    for name in ['spike','reference_spike']:
        ...
        run(['clang++','-O3','-std=c++17',*[f'-I{p}' for p in includes],HERE/f'{name}.cpp','-o',cpp])
        sources=vendor+[HERE/f'{name}.swift']
        if name=='reference_spike': sources += [ROOT/'Mandelbrot/Core/Precision/DeepNumber.swift',ROOT/'Mandelbrot/Core/Precision/ReferenceOrbit.swift']
