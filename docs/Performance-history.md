# Mandelbrot Performance: History

Every performance experiment on the project, oldest first, each dated and
tied to its commit: what was tried, how it was measured, what it cost or
saved, and what was kept.  Discarded experiments stay, because the reasons
they failed still constrain the design.  The current numbers, the method and
how to reproduce a measurement are in [Performance.md](Performance.md).

Entries are written as they were at the time.  Paths have been updated to the
repository's current layout, but some earlier entries describe code that has
since changed -- the viewer's CPU renderers, for instance, were removed in
2.17 -- and some compared images sampled at pixel edges, before every
renderer moved to pixel centres (also 2.17).  Later entries say where an
earlier one was superseded.
...
`evidence/gpu-pipeline/1.4-gpu-pipeline.json`. Legacy counts and all four goldens pass.
...
Samples: `evidence/gpu-pipeline/1.5-smooth-palettes.json`; PNG:
`evidence/gpu-pipeline/smooth-blue-gold.png`. GPU rendering defaults to smooth colouring.
...
This includes readbacks used by assertions. Samples: `evidence/gpu-pipeline/2.1a-tiles.json`.
...
Samples: `evidence/gpu-pipeline/2.1b-parent-fallback.json`. Coarse tiles precede fine tiles;
...
Raw runs: `evidence/gpu-pipeline/2.1c-blending.json`. This is offscreen integration
...
Five isolated Release runs on Apple M1 Pro, using `benchmarks/measure_tiles.py`. Raw
results: `evidence/gpu-pipeline/2.1d-cache.json`. All numerical and cache assertions
...
`evidence/gpu-pipeline/tiles-deep.png` is a 1024×768 tiled FloatFloat export at the
...
traces are in `evidence/gpu-pipeline/uniform-dispatch.json`: median trace 483.178 ms, median per-run p95 compositor GPU duration 0.377 ms, worst deep-refinement GPU batch 2.564 ms. These are host measurements; the device that reported the assertion still needs a rebuilt-app run.

## Review: local ancestor scheduling

*2026-09-14 · `a698104`*

Five M1 Pro traces (`evidence/gpu-pipeline/review-local-levels.json`), now using the
...
Five M1 Pro traces: median per-run p95 compositor GPU time 0.144 ms; worst observed 0.773 ms. Raw: `evidence/gpu-pipeline/review-fallback.json`.
...
traces (`evidence/gpu-pipeline/review-palette-cache.json`): median elapsed 467.361 ms. The trace includes fixed sleeps, so
...
Five M1 Pro traces (`evidence/gpu-pipeline/review-demand.json`): median last measured
...
Five isolated M1 Pro runs: `evidence/gpu-pipeline/review-final.json`. Deep rendering
...
[the reference-library spike](../benchmarks/reference-library/README.md); raw measurements are in
[evidence/perturbation/2.2](evidence/perturbation/2.2). No LGPL dependencies ship in the app.
...
[perturbation-before-bla.json](evidence/perturbation/2.2/perturbation-before-bla.json).
...
[Raw controlled measurements](evidence/perturbation/2.2/bla-comparison.json),
[reproduction script](../benchmarks/measure_bla.py), and full-size PNG evidence:
[1e50](evidence/perturbation/2.2/gpu-1e50.png), [1e200](evidence/perturbation/2.2/gpu-1e200.png),
[1e1000](evidence/perturbation/2.2/gpu-1e1000.png). These are Mac measurements, not claims
...
Generator: `tests/oracles/minibrot_oracle.py`; test: `tests/cli/test_minibrot.py`.
Comparable 64×48, three-run/one-warmup measurements of both locations are stored
in `evidence/perturbation/review/rebasing.json` and reproduced by
`benchmarks/measure_deep_review.py APP STAGE`.
...
`evidence/perturbation/review/geometry.json`; the HUD now separates CPU draw preparation,
...
Raw data: `evidence/perturbation/review/hierarchy.json`. Run
`python3 benchmarks/measure_deep_review.py APP hierarchy` to reproduce all three
...
`python3 benchmarks/reference-library/reproduce.py` fetches the pinned Boost 1.90 repositories,
...
`evidence/perturbation/review/libraries.json`.
...
`evidence/perturbation/review/final.json` includes every timing and component counter.
...
including 27 reference cache hits, are in `evidence/perturbation/review/final-tiles.json`.
...
Final images: [minibrot, full GPU](evidence/perturbation/review/final-gpu.png) and
[independent-golden-sized tiled output](evidence/perturbation/review/final-tiles.png).
...
Raw observations are in `evidence/perturbation/followup/navigation.txt` and
`evidence/perturbation/followup/tile-validation.json`; timings are observations, not CI limits.
...
`python3 benchmarks/measure_deep_followup.py APP` to record all runs and images.
These measure readiness after GPU-context initialisation, not display presentation
or physical-phone frame pacing. Raw data: `evidence/perturbation/followup/measurements.json`.
...
timings are in `evidence/perturbation/review/followup.json`.
...
`evidence/perturbation/followup/bla-errors.json`. These tests measure individual jumps, not
...
[production](evidence/perturbation/followup/minibrot-compound.png),
[fixed-margin candidate](evidence/perturbation/followup/minibrot-fixed.png).
...
(see [Method](Performance.md#method)). The suite was then changed to prime the clock, and the
comparison repeated.
...
pair. Raw samples are in [evidence/float-kernel](evidence/float-kernel/).
...
[cold-renders.json](evidence/gpu-pipeline/tile-batching/cold-renders.json).
...
([compositor-frames.json](evidence/gpu-pipeline/tile-batching/compositor-frames.json)) —
